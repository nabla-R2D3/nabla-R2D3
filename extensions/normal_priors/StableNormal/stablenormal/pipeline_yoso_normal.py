...
class YOSONormalsPipeline(StableDiffusionControlNetPipeline):
    ...
    # @torch.no_grad()## DEBUG FIXME: to enable training mode comments this line.
    ...
    def __call__(
        ...
        manual_mix_precision: bool = True,
        vae_dtype: Optional[torch.dtype] = None,
        unet_dtype: Optional[torch.dtype] = None,
        controlnet_dtype: Optional[torch.dtype] = None,
        ...
        **kwargs,
        ...
        self.empty_text_embedding = torch.zeros(1, 257, 1024).to(device, dtype).expand(num_images, -1, -1)
        ...
        if not skip_preprocess:
            ...
            # image = self.image_processor.load_image_canonical(image, device, dtype)  # [N,3,H,W]
        ...
        # del image # DEBUG FIXME: to enable training mode comments this line.

        # 6. obtain control_output
        if manual_mix_precision and unet_dtype is not None:
            # down_block_res_samples = down_block_res_samples.to(dtype=unet_dtype)
            # mid_block_res_sample = mid_block_res_sample.to(dtype=unet_dtype)
            self.empty_text_embedding = self.empty_text_embedding.to(dtype=unet_dtype)
            pred_latent = pred_latent.to(dtype=unet_dtype)
            image_latent = image_latent.to(dtype=unet_dtype)
        ...
        #     pred_latent,
        #     image_latent,
        # ) # DEBUG FIXME: to enable training mode comments this line.

        # decoder
        
        if manual_mix_precision and vae_dtype is not None:
            latent_x_t = latent_x_t.to(dtype=vae_dtype)
        
        prediction = self.decode_prediction(latent_x_t)
        
        if not skip_postprocess: ## DEBUG FIXME: to enable training mode, this line.
    ...
    def normalize_normals(normals: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
        ...
        normals = normals/norm.clamp(min=eps)
