...
class TextConditionMetrics_NablaR2D3:
    def __init__(self,
        clip_name: str = "openai/clip-vit-base-patch32",
        device:str ="cuda:0",
    ):
        self.image_processor = CLIPImageProcessor.from_pretrained(clip_name)
        self.image_encoder = CLIPVisionModelWithProjection.from_pretrained(clip_name).to(device).eval()

        self.tokenizer = CLIPTokenizer.from_pretrained(clip_name)
        self.text_encoder = CLIPTextModelWithProjection.from_pretrained(clip_name).to(device).eval()
    ...
    def evaluate(self,
        ...
        return clip_sim, clip_rprec
    @torch.no_grad()
    def evaluate_lite(self,
        image: Union[PILImage, List[PILImage]],
        text: Union[str, List[str]],
    ) -> Tuple[float, float, float]:
        if isinstance(image, PILImage):
        ...
        # assert len(image) == len(text)
        assert len(text) == 1, f"len(text)={len(text)}"
        ...
        text_embeds = text_embeds / text_embeds.norm(p=2, dim=-1, keepdim=True)
        assert len(text) == 1, f"len(text)={len(text)}"
        text_embeds= einops.repeat(text_embeds, '1  d -> b  d', b=image_embeds.shape[0])

        assert image_embeds.shape == text_embeds.shape

        clip_scores =image_embeds*text_embeds # (B, N)

        # 1. CLIP similarity
        clip_sim = clip_scores.sum(dim=-1).mean().item()
